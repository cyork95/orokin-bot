...
module.exports = {
	name: 'vox',
	description: 'Get the current Vox Solaris syndicate missions.',
	execute(message) {
		...
		req.end(function(res) {
			...
			const jsonEmbed = new Discord.MessageEmbed()
				.setTitle(`Current Vox Solaris Syndicate Missions for ${platform}`);
			jsonResponse.forEach(mission => {
				mission['jobs'].forEach(job => {
					if(mission['syndicate'] == 'Vox Solaris') {
